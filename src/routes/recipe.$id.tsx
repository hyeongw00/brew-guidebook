  ...
  return (
    <MobileShell>
      <div className="relative">
        <img src={recipe.image} alt={recipe.title} className="aspect-square w-full object-cover" />
      ...
      <div className="px-5 pt-5">
        ...
        {recipe.beanId ? (
          ...
          >
            ...
            <span className="ml-1 text-[10px] font-semibold text-[var(--bean)]">원두 정보 →</span>
          </Link>
        ) : (
      ...
      <section className="px-5 pt-5">
        <h2 className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">레시피</h2>
      ...
      <section className="px-5 pt-6">
        <h2 className="mb-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">단계별 추출</h2>
        <ol className="space-y-2.5">
          {recipe.steps.map((s, i) => (
            <li key={i} className="flex gap-3 rounded-2xl border border-border/60 bg-card p-3">
      ...
      <section className="px-5 pt-6">
        <h2 className="mb-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">맛 프로파일</h2>
        <div className="rounded-2xl border border-border/60 p-4">
          <TasteProfileView taste={recipe.taste} />
          <div className="mt-4 flex flex-wrap gap-1.5">
            {recipe.tastingNotes.map((n) => (
              <span key={n} className="rounded-full bg-[var(--cream)] px-2.5 py-1 text-[11px] font-medium text-[var(--bean)]">
                {n}
              </span>
            ))}
          </div>
      ...
      <section className="px-5 pt-6">
        <h2 className="mb-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">사용 도구</h2>
        <GearChips gear={recipe.gear} />
      </section>

      <section className="px-5 pt-6">
        <h2 className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">노트</h2>
        <p className="rounded-2xl bg-secondary/60 p-4 text-sm leading-relaxed text-foreground">{recipe.review}</p>
      </section>
